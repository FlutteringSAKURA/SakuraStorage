...
// 테스트 코드임 -> 샤크 뱃속 표현을 위한 캠

public class CamViewCtrl : MonoBehaviour
{
  ...
  //  GameObject playerObject;
    ...
    private void LateUpdate()
    {
        ...
        // TEST CODE for View Fix when Jump Action
      //  Ray _jumpCamRay = new Ray(tr.position, Vector3.down * 2.5f);
       // Debug.DrawRay(tr.position, Vector3.down * 2.5f, Color.red);
        // ============================================================= Test Code End

        if (camDist <= dist) // 캠의 거리가 설정한 거리와 같아지거나 짧아지면 
        {
            tr.position = Vector3.Lerp(tr.position, targetTr.position
        ...
        // TEST CODE for View Fix when Jump Action
        // 점프를 하거나 캐릭터가 추락 상태거나 죽은 상태라면
        //if (Input.GetKeyDown(KeyCode.Space) || playerObject.GetComponentInChildren<PlayerCtrl>()._isFalling)
        //{
        //    // 지면 위의 상태가 아니다
        //    _onGround = false;
        //}
        // Jump Cam으로부터의 Ray를 2.8미터 아래로 쏘아 닿는다면
        //else if (Physics.Raycast(_jumpCamRay, out _hit, 2.3f))
        //{
