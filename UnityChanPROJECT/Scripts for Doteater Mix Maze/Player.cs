 ...
 */
    private void CheckMove()
    {
        ...
        //회전
        {
            ...
            {
                addRotationY = -ROTATION_Y_KEY; // 왼쪽으로 회전
            }
            else
            if (Input.GetKey(KeyCode.E))
            {
                addRotationY = ROTATION_Y_KEY; // 오른쪽으로 회전
            }
            ...
            //transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.Euler(0, m_rotationY, 0), rotDamp);
        }
        ...
        Vector3 addPosition = Vector3.zero;     //이동량(z 값은 메카님에도 넘겨준다)
        {
            ...
            //키 조작에서 이동할 양을 얻는다
            Vector3 vecInput = new Vector3(0.0f, 0.0f, Input.GetAxisRaw("Vertical"));        // 앞뒤 방향 입력 값을 받아 Z에 넣는다([W], [S] 게임 패드 입력 등)
            // GetAxisRaw (-1) or (0) or (1)  // GetAxis = (-1) - (0) - (1) 사이의 실수값..
